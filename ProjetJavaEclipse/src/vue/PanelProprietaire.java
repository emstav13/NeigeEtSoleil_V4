...
public class PanelProprietaire extends PanelPrincipal implements ActionListener {
	...
	public PanelProprietaire() {
		...
		this.panelForm.setBorder(
				BorderFactory.createCompoundBorder(new LineBorder(Color.LIGHT_GRAY, 2), new EmptyBorder(15, 15, 15, 15) // marges
																														// internes
				));
		...
		if (e.getSource() == this.btAnnuler) {
			...
			try {
				...
				if (Controleur.verifDonnees(lesChamps)) {
					...
					JOptionPane.showMessageDialog(this, "Insertion réussie du Propriétaire.", "Insertion Propriétaire",
							JOptionPane.INFORMATION_MESSAGE);
					actualiserTableau();
				} else {
					JOptionPane.showMessageDialog(this, "Veuillez remplir tous les champs.", "Erreur de saisie",
							JOptionPane.ERROR_MESSAGE);
				}
			} catch (NumberFormatException ex) {
				JOptionPane.showMessageDialog(this, "ID Utilisateur doit être un nombre valide.", "Erreur de saisie",
						JOptionPane.ERROR_MESSAGE);
			}
			this.viderChamps();
			...
			if (selectedRow >= 0) {
				...
				JOptionPane.showMessageDialog(this, "Suppression réussie du propriétaire.", "Suppression",
						JOptionPane.INFORMATION_MESSAGE);
				actualiserTableau();
				viderChamps();
			} else {
				JOptionPane.showMessageDialog(this, "Veuillez sélectionner un propriétaire à supprimer.",
			...
			if (selectedRow >= 0) {
				try {
					int idProprietaire = Integer
							.parseInt(this.tableProprietaires.getValueAt(selectedRow, 0).toString());
					int idUtilisateur = Integer.parseInt(this.txtIdUtilisateur.getText());
					String logements = this.txtLogements.getText();
					...
					JOptionPane.showMessageDialog(this, "Modification réussie du propriétaire.", "Modification",
							JOptionPane.INFORMATION_MESSAGE);
					actualiserTableau();
					viderChamps();
				} catch (NumberFormatException ex) {
					JOptionPane.showMessageDialog(this, "ID Utilisateur doit être un nombre valide.",
							"Erreur de saisie", JOptionPane.ERROR_MESSAGE);
				}
			} else {
				JOptionPane.showMessageDialog(this, "Veuillez sélectionner un propriétaire à modifier.",
						"Erreur de sélection", JOptionPane.ERROR_MESSAGE);
			}
		}
	}
