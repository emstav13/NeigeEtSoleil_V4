...
public class PanelLogement extends PanelPrincipal implements ActionListener {
	...
	public PanelLogement() {
		...
		this.panelRecherche.setBorder(
				BorderFactory.createCompoundBorder(new LineBorder(Color.LIGHT_GRAY, 2), new EmptyBorder(5, 10, 5, 10)));
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource() == this.btAnnuler) {
			...
			try {
				...
				Logement unLogement = new Logement(0, idProprietaire, nomImmeuble, adresse, codePostal, ville,
						typeLogement, surfaceHabitable, capaciteAccueil, specifite, photo);
				Controleur.insertLogement(unLogement);

				JOptionPane.showMessageDialog(this, "Insertion réussie du Logement.", "Insertion Logement",
			...
			if (selectedRow >= 0) {
				...
				JOptionPane.showMessageDialog(this, "Suppression réussie du logement.", "Suppression",
						JOptionPane.INFORMATION_MESSAGE);
				actualiserTableau();
				viderChamps();
			} else {
				JOptionPane.showMessageDialog(this, "Veuillez sélectionner un logement à supprimer.",
			...
			if (selectedRow >= 0) {
				try {
					...
					Logement unLogement = new Logement(idLogement, idProprietaire, nomImmeuble, adresse, codePostal,
							ville, typeLogement, surfaceHabitable, capaciteAccueil, specifite, photo);
					Controleur.updateLogement(unLogement);

					JOptionPane.showMessageDialog(this, "Modification réussie du logement.", "Modification",
							JOptionPane.INFORMATION_MESSAGE);
					actualiserTableau();
					viderChamps();
				} catch (Exception ex) {
				...
				JOptionPane.showMessageDialog(this, "Veuillez sélectionner un logement à modifier.",
						"Erreur de sélection", JOptionPane.ERROR_MESSAGE);
			}
		}
	}
