...
(function() {
  ...
  document.getElementById("formInscription").addEventListener("submit", function (e) {
    ...
    fetch("http://localhost:8080/NeigeEtSoleil_V4/inscription", {
      method: "POST",
      headers: {
      ...
      .then((response) => {
          if (!response.ok) {
              // Si le statut HTTP est une erreur, rejette la promesse
              throw new Error(`HTTP error! status: ${response.status}`);
          }
          return response.json(); // Si tout va bien, parse le JSON
      })
      .then((data) => {
